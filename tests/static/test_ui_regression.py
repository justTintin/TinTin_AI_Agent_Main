...
class TestUIRegression(unittest.TestCase):
            ...
            with open(fp, encoding="utf-8", errors="replace") as f:
                for i, line in enumerate(f, 1):
                    for pat in BARE_READY_PATTERNS:
                        if pat.search(line):
                            bad.append(f"{os.path.basename(fp)}:{i}: {line.strip()}")
        self.assertEqual(bad, [], "仍存在无意义的默认「就绪」文案:\n" + "\n".join(bad))

    def test_compile_video_subtitle_above_folder(self):
        with open(os.path.join(GUI_DIR, "compile_video_page.py"), encoding="utf-8") as f:
            src = f.read()
        lines = src.splitlines()
        line_sub = next(i for i, line in enumerate(lines, 1) if "self.in_subtitle = QTextEdit" in line)
        line_folder = next(i for i, line in enumerate(lines, 1) if "self.in_folder = self._file_row" in line)
    ...
    def test_storyboard_auto_json(self):
        with open(os.path.join(GUI_DIR, "storyboard_page.py"), encoding="utf-8") as f:
            src = f.read()
...
class TestMenuStructure(unittest.TestCase):
    ...
    def _sidebar_src(self):
        with open(os.path.join(testutil.STUDIO_DIR, "gui", "main_window_sidebar.py"),
                  encoding="utf-8") as f:
            return f.read()
    ...
    def test_media_tools_has_nine_cards(self):
        with open(os.path.join(testutil.STUDIO_DIR, "gui", "media_tools_page.py"),
                  encoding="utf-8") as f:
            src = f.read()
