...
from config.paths import QWEN_IMAGE_LAYERED_DIR, TMP_DIR
...
class QwenLayeredWorker(BaseWorker):
    progress = Signal(str)
    finished = Signal(bool, list, str, str, str, str) # success, layer_paths, pptx_path, zip_path, psd_path, error_msg  # noqa: E501

    def __init__(self, img_path, layers_count=4, num_steps=50, scale=4.0, prompt="", neg_prompt=" ", seed=-1, low_vram=True):  # noqa: E501
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if not torch.cuda.is_available():
                raise RuntimeError("当前系统缺少 NVIDIA CUDA 支持。图像智能分层模型体量巨大，必须在 NVIDIA GPU 下执行！")  # noqa: E501
            ...
            if _cached_pipeline is None:
                if os.path.exists(os.path.join(qwen_repo_dir, "model_index.json")):
                    self.progress.emit("正在从本地目录 (apps/Qwen-Image-Layered) 载入 Qwen-Image-Layered 预训练模型...")  # noqa: E501
                    model_path = qwen_repo_dir
                elif os.path.exists(os.path.join(os.path.dirname(qwen_repo_dir), "Qwen-Image-Layered-Weights", "model_index.json")):  # noqa: E501
                    self.progress.emit("正在从本地目录 (apps/Qwen-Image-Layered-Weights) 载入 Qwen-Image-Layered 预训练模型...")  # noqa: E501
                    model_path = os.path.join(os.path.dirname(qwen_repo_dir), "Qwen-Image-Layered-Weights")  # noqa: E501
                else:
                    os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"
                    self.progress.emit("正在从云端下载并载入 Qwen-Image-Layered 预训练模型 (模型约 45GB，初次使用国内镜像高速下载，请保持联网)...")  # noqa: E501
                    model_path = "Qwen/Qwen-Image-Layered"

                _cached_pipeline = QwenImageLayeredPipeline.from_pretrained(model_path, torch_dtype=torch.bfloat16)  # noqa: E501
            ...
            def imagelist_to_pptx(img_files):
                ...
                for path in img_files:
                    slide.shapes.add_picture(path, left, top, width=px_to_emu(w_px), height=px_to_emu(h_px))  # noqa: E501
            ...
            self.progress.emit(f"AI 正在开始进行图层分层解耦 (图层数:{self.layers_count}, 步数:{self.num_steps})...")  # noqa: E501
        ...
        except Exception as e:  # AI model inference
...
class ResultPreviewWidget(QWidget):
    ...
    def paintEvent(self, event):  # noqa: N802
        ...
        W, H = self.width(), self.height()  # noqa: N806
...
class LayerCardWidget(QFrame):
    def __init__(self, layer_idx, img_path, on_preview, on_save):
        ...
        self.setStyleSheet("QFrame#card { background-color: #2c2c2e; border: 1px solid #3a3a3c; border-radius: 10px; }")  # noqa: E501
...
class ImageLayeredPage(BasePage):
    ...
    def setup(self):
        ...
        if not os.path.exists(QWEN_IMAGE_LAYERED_DIR):
            ...
            msg_label.setStyleSheet("font-size: 18px; font-weight: bold; color: #ff9900;")  # noqa: E501
            ...
            desc_label.setStyleSheet("font-size: 13px; color: #8e8e93; line-height: 20px;")  # noqa: E501
        ...
        self.chk_low_vram.setToolTip("开启后将启用 CPU Offload 机制，仅在计算时将必要图层载入显存，大幅降低显存需求，防止 24G/16G/12G/8G 显卡发生 Out of Memory 错误")  # noqa: E501
        ...
        self.scroll_layers.setStyleSheet("QScrollArea { border: none; background: transparent; }")  # noqa: E501
        ...
        if file_path:
            ...
            if not img.isNull():
                self.lbl_image_info.setText(
                    f"图片路径: {os.path.basename(file_path)}\n分辨率: {img.width()} x {img.height()}"  # noqa: E501
                )
    ...
    def on_worker_finished(self, success, layer_paths, pptx_path, zip_path, psd_path, err_msg):  # noqa: E501
        self.progress_bar.setVisible(False)
        self.btn_run.setEnabled(True)
        ...
        if success:
            ...
            for idx, path in enumerate(self.layer_paths):
                card = LayerCardWidget(idx + 1, path, self.on_layer_preview, self.on_layer_save)  # noqa: E501
                row = idx // col_count
                col = idx % col_count
            ...
            QMessageBox.information(self.parent, "分层成功", f"图像已成功解耦为 {len(layer_paths)} 个透明图层，您可以通过下方按钮导出整图工程，或单独导出/预览各图层。")  # noqa: E501
        else:
    ...
    def save_psd(self):
        if not self.psd_path or not os.path.exists(self.psd_path):
            return
        save_path, _ = pick_save_file(
            self.parent, "另存为 Photoshop PSD 工程图层文件", "decomposed_image.psd", "Photoshop 文件 (*.psd)"  # noqa: E501
        )
        if save_path:
            try:
