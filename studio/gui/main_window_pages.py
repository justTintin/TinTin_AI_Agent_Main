...
class PageSetupMixin:
    ...
    def setup_llm_settings_page(self):
        ...
        self.comfyui_input.setPlaceholderText("留空=本地 X.X.X.X:8188；或填外部如 http://X.X.X.X:8188")  # noqa: E501
        self.comfyui_input.setText(self.ai_config.get("comfyui_addr","http://X.X.X.X:8188"))
        l1.addWidget(self.comfyui_input)  # noqa: E501
        self.comfyui_local_status = QLabel()
        self.comfyui_local_status.setObjectName("comfyui_local_status")
        l1.addWidget(self.comfyui_local_status)  # noqa: E501
        with contextlib.suppress(Exception):
            self.refresh_comfyui_local_status()
        self.btn_open_comfyui_editor = QPushButton(" 打开 ComfyUI 节点编辑器（调试工作流）")
        self.btn_open_comfyui_editor.clicked.connect(self.open_comfyui_editor)
        l1.addWidget(self.btn_open_comfyui_editor)  # noqa: E501
        r1 = QHBoxLayout()
        r1.addStretch()
        b_save_comfy = mdi_button("保存 ComfyUI 配置", "save")
        b_save_comfy.setObjectName("primary_button")
        b_save_comfy.clicked.connect(self.save_ai_config)
        r1.addWidget(b_save_comfy)  # noqa: E501
        l1.addLayout(r1)
        l1.addStretch()
        tab_bar.addTab(" ComfyUI")
        stack.addWidget(p1)
        # ── Tab 2: RunningHub ──
        # 配置项较多：外层包滚动容器，窗口偏矮时也能完整查看不截断
        p2 = QWidget()
        l2_outer = QVBoxLayout(p2)
        l2_outer.setContentsMargins(0, 0, 0, 0)  # noqa: E501
        l2_scroll = QScrollArea()
        l2_scroll.setWidgetResizable(True)
        l2_scroll.setStyleSheet("QScrollArea { border: none; background: transparent; }")  # noqa: E501
        l2_outer.addWidget(l2_scroll)
        _p2_inner = QWidget()
        l2 = QVBoxLayout(_p2_inner)
        l2.setContentsMargins(30,30,30,30)  # noqa: E501
        l2_scroll.setWidget(_p2_inner)
        l2.addWidget(QLabel("RunningHub API Key（在 RunningHub 个人中心获取）:"))
        self.runninghub_api_key_input = QLineEdit()
        self.runninghub_api_key_input.setPlaceholderText("rh_xxxxxxxx...")
        self.runninghub_api_key_input.setEchoMode(QLineEdit.Password)
        self.runninghub_api_key_input.setText(self.ai_config.get("runninghub_api_key", ""))
        l2.addWidget(self.runninghub_api_key_input)  # noqa: E501
        l2.addWidget(QLabel("RunningHub 基础地址（一般保持默认）:"))
        self.runninghub_base_url_input = QLineEdit()
        self.runninghub_base_url_input.setPlaceholderText("https://www.runninghub.cn")
        self.runninghub_base_url_input.setText(self.ai_config.get("runninghub_base_url", "https://www.runninghub.cn"))
        l2.addWidget(self.runninghub_base_url_input)  # noqa: E501
        l2.addWidget(QLabel("在线工作流 ComfyUI 协议会话凭证（登录浏览器控制台 localStorage 获取）:"))
        self.runninghub_comfy_auth_input = QLineEdit()
        self.runninghub_comfy_auth_input.setPlaceholderText("Rh-Comfy-Auth")
        self.runninghub_comfy_auth_input.setToolTip("localStorage.getItem('Rh-Comfy-Auth')")
        self.runninghub_comfy_auth_input.setText(self.ai_config.get("runninghub_comfy_auth", ""))
        l2.addWidget(self.runninghub_comfy_auth_input)  # noqa: E501
        self.runninghub_comfy_identify_input = QLineEdit()
        self.runninghub_comfy_identify_input.setPlaceholderText("Rh-Identify")
        self.runninghub_comfy_identify_input.setToolTip("localStorage.getItem('Rh-Identify')")
        self.runninghub_comfy_identify_input.setText(self.ai_config.get("runninghub_comfy_identify", ""))
        l2.addWidget(self.runninghub_comfy_identify_input)  # noqa: E501
        self.runninghub_access_token_input = QLineEdit()
        self.runninghub_access_token_input.setPlaceholderText("Rh-Accesstoken")
        self.runninghub_access_token_input.setToolTip("localStorage.getItem('Rh-Accesstoken')")
        self.runninghub_access_token_input.setText(self.ai_config.get("runninghub_access_token", ""))
        l2.addWidget(self.runninghub_access_token_input)  # noqa: E501
        self.runninghub_use_personal_queue_check = QCheckBox("使用个人独占队列（企业级-独占 Key 建议开启）")
        self.runninghub_use_personal_queue_check.setToolTip("usePersonalQueue；独占机器请保持勾选，否则任务可能不会被分配到你租的机器")
        self.runninghub_use_personal_queue_check.setChecked(bool(self.ai_config.get("runninghub_use_personal_queue", True)))
        l2.addWidget(self.runninghub_use_personal_queue_check)  # noqa: E501
        r2 = QHBoxLayout()
        r2.addStretch()
        b_save_rh = mdi_button("保存 RunningHub 配置", "save")
        b_save_rh.setObjectName("primary_button")
        b_save_rh.clicked.connect(self.save_ai_config)
        r2.addWidget(b_save_rh)  # noqa: E501
        b_test_rh = mdi_button("测试连接", "link")
        b_test_rh.setObjectName("secondary_button")
        b_test_rh.clicked.connect(self._test_runninghub_config)
        r2.addWidget(b_test_rh)  # noqa: E501
        b_test_comfy = mdi_button("测试 ComfyUI 协议", "link")
        b_test_comfy.setObjectName("secondary_button")
        b_test_comfy.clicked.connect(self._test_runninghub_comfy_protocol)
        r2.addWidget(b_test_comfy)  # noqa: E501
        l2.addLayout(r2)
        self.rh_config_status = QLabel("")
        self.rh_config_status.setObjectName("muted_text")
        l2.addWidget(self.rh_config_status)  # noqa: E501
        self.rh_comfy_status = QLabel("")
        self.rh_comfy_status.setObjectName("muted_text")
        l2.addWidget(self.rh_comfy_status)  # noqa: E501

        # ── RunningHub 工作流管理 ──
        l2.addSpacing(20)
        wf_group = QGroupBox(" 工作流管理")
        wf_group.setObjectName("model_groupbox")
        ...
        self.rh_workflow_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)  # noqa: E501
        self.rh_workflow_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)  # noqa: E501
        self.rh_workflow_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)  # noqa: E501
        self.rh_workflow_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeToContents)  # noqa: E501
        ...
        self.rh_workflow_table.itemSelectionChanged.connect(self._rh_on_workflow_selection_changed)  # noqa: E501
