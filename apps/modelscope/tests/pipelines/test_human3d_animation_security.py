...
from modelscope.pipelines.cv.human3d_animation_pipeline import \
    Human3DAnimationPipeline  # noqa: E402
...
if _PKG not in sys.modules or not hasattr(sys.modules[_PKG],
                                          'gen_skeleton_bvh'):
    try:
...
class Human3DAnimationSecurityTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.untrusted_dir = os.path.join(self.tmp_root, 'attacker', 'badrepo')
