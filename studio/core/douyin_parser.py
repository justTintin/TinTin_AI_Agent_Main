...
def parse_video_detail_json(aweme_id, json_res):
    ...
    title = title.replace('\\n', ' ').replace('\n', ' ').replace('\\t', ' ').replace('\t', ' ').replace('\\r', ' ').replace('\r', ' ').replace('\\', '')  # noqa: E501
    ...
    temp = {
        ...
        "normalVideo": None,
        "audio": None,
        'imageUrl': res_data.get('video', {}).get('cover', {}).get('url_list', [None])[0],  # noqa: E501
        ...
        'publishTime': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(res_data.get('create_time', time.time()))),  # noqa: E501
    ...
    if bit_rates:
        ...
        sorted_bit_rates = sorted(bit_rates, key=lambda x: x.get('bit_rate', 0), reverse=True)  # noqa: E501
