...
from .dialogs import PluginDownloadDialog
from .widgets import (
    _GridRowsFilter,
    _MAX_THUMB_WORKERS,
    _THUMB_ICON_SIZE,
...
class VectorSearchPage(BasePage):
    def setup(self):
        ...
        sidebar.setFixedWidth(280)  # 固定宽度，避免 QSplitter 初始/用户拖拽后变宽
        ...
        self._init_retry_count = 0       # 初始化重试计数
        ...
        # 初始：等待窗口布局稳定后再加载（避免非最大化时列宽计算错误）
        QTimer.singleShot(200, self._init_load_if_ready)
    ...
    #  初始化：统计 / 筛选项 / 默认浏览
    # ══════════════════════════════════════════
    def _init_load_if_ready(self):
        """等待窗口布局稳定后再加载数据，避免视口宽度为0时计算列宽错误。"""
        vp = self.grid.viewport()
        w = vp.width() if vp else 0
        # 检查视口宽度是否合理（至少 200px 才能正确计算10列）
        if w < 200 and self._init_retry_count < 20:
            self._init_retry_count += 1
            QTimer.singleShot(100, self._init_load_if_ready)
            return
        # 窗口已稳定，确保 gridSize 正确后再加载
