...
def get_ab(target_url, user_agent, cookie_str):
    for _index in range(2):
        try:
            p = subprocess.Popen(['node', './assets/douyin_a_bogus.js', target_url, user_agent, cookie_str], stdout=subprocess.PIPE)  # noqa: E501
            if p.stdout is None:
                return False
            result = p.stdout.read().strip().decode('utf-8')
            match = re.search(r'a_bogus: (.+)', result)
            if match is None:
                raise AttributeError("no a_bogus match")
            a_bogus = match.group(1)
            return a_bogus
        except (OSError, subprocess.SubprocessError, UnicodeDecodeError, AttributeError):  # noqa: E501
            pass
