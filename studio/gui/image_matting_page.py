...
from config.paths import REMBG_DIR, TMP_DIR
...
class RembgWorker(BaseWorker):
    ...
    def run(self):
        ...
        except Exception as e:  # 外部库调用（PIL + rembg），涉及多类异常
...
class ImageMattingPage(BasePage):
    ...
    def setup(self):
        ...
        tips = QLabel(" 提示：本功能基于深度学习，完全自动抠除背景。首次使用对应模型时会自动联网下载权重文件到用户主目录下的 .u2net 文件夹中，请保持网络畅通。")  # noqa: E501
    ...
    def run_matting(self):
        ...
        self.matting_worker = RembgWorker(self.selected_img_path, self.output_temp_path, model_name)  # noqa: E501
