...
from . import douyin_a_bogus, douyin_video


class DouyinUserDownloader:
    def __init__(self, user_url, cookie_file='douyin_cookies.txt', download_path='downloads', cookie_str=None):  # noqa: E501
    ...
    def _extract_sec_uid(self):
        ...
        if 'v.douyin.com' in url or '/user/' in url:
            ...
            try:
                ...
                res = http_get(url, headers=self.headers, allow_redirects=True, timeout=10)  # noqa: E501
                url = res.url
    ...
    def fetch_all_videos(self):
        ...
        fp_val = fp_match.group(1) if fp_match else 'verify_lw66uj9x_y69HTWBr_NOK0_4O0k_As0k_vZiOREH3U5SC'  # noqa: E501
        ...
        while has_more:
            msToken = douyin_video.generate_random_str()  # type: ignore[attr-defined]  # noqa: N801 N806
            url = (
                ...
                f"&publish_video_strategy_type=2&pc_client_type=1&version_code=170400&version_name=17.4.0"  # noqa: E501
                f"&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=zh-CN"  # noqa: E501
                f"&browser_platform=Win32&browser_name=Chrome&browser_version=131.0.0.0"
                f"&browser_online=true&engine_name=Blink&engine_version=131.0.0.0&os_name=Windows"  # noqa: E501
                f"&os_version=10&cpu_core_num=16&device_memory=8&platform=PC&downlink=10"  # noqa: E501
                f"&effective_type=4g&round_trip_time=0&webid={webid_val}"
                f"&verifyFp={fp_val}&fp={fp_val}&msToken={msToken}"
            )

            query_params = url.split('?')[1]
            ab = douyin_a_bogus.get_ab(target_url=query_params, user_agent=self.headers['user-agent'], cookie_str=self.cookies_str)  # noqa: E501
            if ab:
                url += f"&a_bogus={ab}"
            ...
            try:
                ...
                try:
                    ...
                    if "verify" in res.text or "验证" in res.text:
                        raise Exception("触发了验证码，请在浏览器中手动验证后再重试") from e
                    if "登录" in res.text or "login" in res.text:
                        raise Exception("检测到已退出登录，请重新登录") from e
                    raise Exception(f"API 响应内容不是有效的 JSON 格式 (HTTP {res.status_code})") from e
            ...
            except Exception as e:  # HTTP 请求 + JSON 解析 + 数据处理
    ...
    def _fetch_single_video(self, aweme_id):
        ...
        try:
            ...
            # that the rest of the code (download_videos, display_parsed_videos) expects.  # noqa: E501
            create_time = 0
            try:
                from datetime import datetime
                dt = datetime.strptime(detail.get('publishTime', ''), '%Y-%m-%d %H:%M:%S')  # noqa: E501
            ...
            aweme_item = {
                ...
                'video': {
                    'play_addr': {
                        'url_list': [detail.get('originVideo')] if detail.get('originVideo') else []  # noqa: E501
                    }
                }
            }
            logger.info(f"成功获取单个视频: {aweme_item['desc']}")
            return [aweme_item]
        except Exception as e:  # 外部API调用（抖音视频详情获取）
    ...
    def download_videos(self, aweme_list):
        ...
        for idx, item in enumerate(aweme_list):
            ...
            time_str = time.strftime('%Y%m%d_%H%M%S', time.localtime(ctime)) if ctime else "00000000"  # noqa: E501
            ...
            except Exception as e:  # 外部API调用（视频地址获取 + 下载）
            ...
            if res.status_code == 200:
                with open(filepath, 'wb') as f:
                    for chunk in res.iter_content(chunk_size=8192):
                        f.write(chunk)
                return True
        except Exception as e:  # HTTP 下载 + 文件写入
